...
function setupEventListeners() {
    ...
    document.addEventListener("click", function (e) {
        ...
        if (watchBtn) {
            ...
            const slug = watchBtn.dataset.slug;
            const title = watchBtn.dataset.title;

            if (!slug) {
                console.warn("Watch tanpa slug:", watchBtn);
                return;
            }

            loadAnimeDetail(slug, title);
            return;
        }
        ...
        if (card) {
            ...
            if (!slug) {
                console.warn("Card tanpa slug:", card);
                return;
            }
        ...
        // Episode
        const ep = e.target.closest(".episode-card");
        if (ep) {
            loadEpisode(ep.dataset.slug, ep.dataset.title);
            return;
        }
        ...
        if (tag) {
            e.preventDefault();
            loadAnimeByGenre(tag.dataset.slug, tag.textContent);
            return;
        }
...
async function loadAnimeDetail(slug, title) {
    ...
    try {
        ...
        if (data.data) {
            ...
            const ep1 = await fetchEpisode(epWatch);
...
function getAnimeSlug(anime) {
    const slug =
        ...
        anime?.link?.split("/").filter(Boolean).pop() ||
        anime?.synopsis?.connections?.[0]?.animeId ||
        null;

    if (!slug) {
        console.warn("Anime tanpa slug:", anime);
    }
...
function createAnimeCardElement(anime) {
    ...
    if (!slug) return document.createElement("div");

    const card = document.createElement("div");
    card.className = "anime-card";
    card.dataset.slug = slug;
    card.innerHTML = createAnimeCardHTML(anime, slug);
